...
try {
  ...
  assert.equal((await post('/api/login', { password: 'test-only-password-123', inventoryMode: 'products' })).status, 200)
  assert.equal((await rpc('products.list')).length, 3)
  await post('/api/logout', {})
  assert.equal((await post('/api/login', { password: 'test-only-password-123', inventoryMode: 'agrochemicals' })).status, 200)
  assert.equal((await rpc('products.list')).length, 0)
  tree = await rpc('categoryManager.list')
  assert.deepEqual(tree.map((category) => category.name).sort(), ['OP', 'โกโก้', 'ส่งเสริม'].sort())
  await rpc('categoryManager.createMain', 'ปุ๋ย')
  tree = await rpc('categoryManager.list')
  await rpc('categoryManager.createSubcategory', tree[0].id, 'ปุ๋ยเม็ด')
  await rpc('products.create', { ...input, name: 'ปุ๋ยทดสอบ', category: 'ปุ๋ย', subcategory: 'ปุ๋ยเม็ด' })
  assert.equal((await rpc('products.list')).length, 1)
  const agroExport = await fetch(origin + '/api/export?format=csv', { headers: { Cookie: cookie } })
  const agroCsv = await agroExport.text()
  assert.match(agroCsv, /วันที่สั่งเข้ามา/)
  assert.doesNotMatch(agroCsv, /วันหมดอายุ/)
  await post('/api/logout', {})
  assert.equal((await post('/api/login', { password: 'test-only-password-123', inventoryMode: 'products' })).status, 200)
  assert.equal((await rpc('products.list')).length, 3)
  await post('/api/logout', {})
  assert.equal((await post('/api/rpc', { method: 'products.list' })).status, 401)
  console.log('PASS: authentication, inventory separation, CSRF, CRUD, concurrent stock, stale edits, encrypted tokens, CSV/XLSX, persistence and logout')
} finally { await stop() }
